...
- `src/options.html`, `src/options.js` — простая страница настроек для ввода API-ключа.
...
4. Откройте страницу настроек расширения (клик по «Подробнее» → «Параметры») и сохраните API-ключ Google Vision. Можно также задать его вручную в консоли сервис-воркера через `globalThis.VISION_API_KEY`.
5. Перейдите на любую страницу и наведите курсор на изображение или видео — появится панель действий.
